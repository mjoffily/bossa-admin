...
export function fetchPurchaseOrderCmd(id, originalMsg) {
    return {
...
export function httpPurchaseOrderFetchedMsg(response) {
    return {
        msg: MSGS.HTTP_FETCH_PURCHASE_ORDER_SUCCESS,
        response
    }
}

function httpErrorFetchingPurchaseOrderMsg(error, originalMsg) {
    return {
...
function httpProductsFetchedMsg(response) {
    return {
        msg: MSGS.HTTP_FETCH_PRODUCTS_SUCCESS,
        response
    };
}
...
function httpPurchaseOrderSavedSuccessfullyMsg(response) {
    return {
        msg: MSGS.HTTP_PURCHASE_ORDER_SAVED_SUCCESS,
        response
    };
}
...
export function update(msg, model) {
    switch (msg.msg) {
        ...
        case MSGS.RETRIEVE_PURCHASE_ORDER:
            {
                const commands = [fetchPurchaseOrderCmd(msg.id, msg), fetchProductsCmd];
        ...
        case MSGS.HTTP_ERROR:
            {
                const response = R.pathOr({}, ['response'], msg.error)
                const messagesToPropagate = [navigateToPageMsg(PAGES.LOGIN), redirectToMsg(PAGES.PO_DETAIL, msg.originalMsg)];
